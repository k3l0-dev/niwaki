"""Tenant — static path bindings on every interface of the first leaf (non-prod).

Run:
    uv run pytest tests/integration/04_tenant/test_011_static_paths_leaf_a.py -m integration -s

A static-path binding (``fvRsPathAtt``) for **every front-panel interface** of
the first discovered leaf, data-driven from ``l1PhysIf`` at runtime — one per
...
from this domain's VLAN lane (vlan-2500..2599), reused across ports (the only
conflict is with a VLAN pool, never across ports). The first ports carry the
leaf-port children: port security, IGMP/MLD snoop static and access groups, and
an NLB static group (multicast MAC).

Values are illustrative. This file owns tenant ``niwaki-it-sp-a``; ``wipe``
...
TN = "niwaki-it-sp-a"
VRF = "niwaki-it-sp-a-vrf"
BD = "niwaki-it-sp-a-bd"
APP = "niwaki-it-sp-a-app"
LEAF_INDEX = 0
...
def test_static_paths_all_interfaces(live_aci: Niwaki) -> None:
    """A static-path binding per front-panel interface of the first leaf."""
    tn = tenant(TN, description="Static-path bindings on every interface of the first leaf")
    ...
    if leaf is None or not ifaces:
        pytest.skip("no leaf / interfaces discovered on the fabric")
    ...
    for port, iface in enumerate(ifaces, start=1):
        ...
        if port <= 3:
            ...
            path.igmp_snoop_static_group(
                f"225.60.{port}.1", f"10.60.{port}.5", description="IGMP snoop static group."
            )
            path.igmp_snoop_access_group(description="IGMP snoop access group.")
            path.mld_snoop_static_group(
                f"ff05::60:{port}", f"2001:db8:60::{port}", description="MLD snoop static group."
            )
            path.mld_snoop_access_group(description="MLD snoop access group.")
            path.nlb_static_group(
                f"03:bf:0a:3c:{port:02d}:01", description="NLB static group (multicast)."
            )
