...
| | |
| --- | --- |
| ACI class | `fvnsVlanInstP` |
| RN | `vlanns-[{name}]-{allocation_mode}` |
| Cursor | `VlanPoolCursor` |
| Parent | {ref}`infra <vocab-infra>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.vlan_pool(name, allocation_mode)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
...
| --- | --- | --- |
| `.range(from_, to, **attrs)` | `fvnsEncapBlk` | {ref}`infra.vlan_pool.range <vocab-infra-vlan_pool-range>` |
