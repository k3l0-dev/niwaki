...
| | |
| --- | --- |
| ACI class | `fvnsVxlanInstP` |
| RN | `vxlanns-{name}` |
| Cursor | `VxlanPoolCursor` |
| Parent | {ref}`infra <vocab-infra>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.vxlan_pool(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.

| parameter | wire | type | values | default | description |
| --- | --- | --- | --- | --- | --- |
| `name` *(positional)* | `name` | `str` | — | — | The VxLAN policy name. |
...
| --- | --- | --- |
| `.range(from_, to, **attrs)` | `fvnsEncapBlk` | {ref}`infra.vxlan_pool.range <vocab-infra-vxlan_pool-range>` |
