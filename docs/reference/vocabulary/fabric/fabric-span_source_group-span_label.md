...
(vocab-fabric-span_source_group-span_label)=

# `fabric.span_source_group.span_label`
...
| RN | `spanlbl-{name}` |
| Cursor | `FabricSpanSourceGroupSpanLabelCursor` |
| Parent | {ref}`fabric.span_source_group <vocab-fabric-span_source_group>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.span_label(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.

| parameter | wire | type | values | default | description |
| --- | --- | --- | --- | --- | --- |
| `name` *(positional)* | `name` | `str` | — | — | The name of the label that is used to match SPAN sources with SPAN destination(s). |
