...
| ACI class | `vnsDevMgr` |
| RN | `devMgr-{name}` |
| Cursor | `DeviceManagerCursor` |
| Parent | {ref}`tenant <vocab-tenant>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.device_manager(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
...
| --- | --- | --- |
| `.credentials(**attrs)` | `vnsCCred` | {ref}`tenant.device_manager.credentials <vocab-tenant-device_manager-credentials>` |
| `.management_interface(ip_address, **attrs)` | `vnsCMgmts` | {ref}`tenant.device_manager.management_interface <vocab-tenant-device_manager-management_interface>` |
