...
(vocab-tenant-l3out-external_epg-vz_prov_subject_label)=
...
| RN | `provsubjlbl-{name}` |
| Cursor | `VzProvSubjectLabelCursor` |
| Parent | {ref}`tenant.l3out.external_epg <vocab-tenant-l3out-external_epg>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.vz_prov_subject_label(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
