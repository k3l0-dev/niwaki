...
(vocab-tenant-vrf-dns_label)=

# `tenant.vrf.dns_label`
...
| ACI class | `dnsLbl` |
| RN | `dnslbl-{name}` |
| Cursor | `DnsLabelCursor` |
| Parent | {ref}`tenant.vrf <vocab-tenant-vrf>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.dns_label(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
