...
(vocab-tenant-management_profile-in_band_epg-provider_contract_label)=

# `tenant.management_profile.in_band_epg.provider_contract_label`

A label identifying a contract. A contract label can be parented by the relation between an endpoint group (EPG) and security group. The EPG is associated with a group and lists all contracts it provides out of the group, as well as, optionally, contracts that it chooses to consume.

| | |
| --- | --- |
| ACI class | `vzProvCtrctLbl` |
| RN | `pCtrctLbl-{name}` |
| Cursor | `InBandEpgProviderContractLabelCursor` |
| Parent | {ref}`tenant.management_profile.in_band_epg <vocab-tenant-management_profile-in_band_epg>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.provider_contract_label(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
