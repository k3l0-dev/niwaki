...
(vocab-tenant-management_profile-in_band_epg)=

# `tenant.management_profile.in_band_epg`
...
| ACI class | `mgmtInB` |
| RN | `inb-{name}` |
| Cursor | `InBandEpgCursor` |
| Parent | {ref}`tenant.management_profile <vocab-tenant-management_profile>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.in_band_epg(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.

| parameter | wire | type | values | default | description |
| --- | --- | --- | --- | --- | --- |
| `name` *(positional)* | `name` | `str` | — | — | The in-band management endpoint group name. This name can be up to 64 alphanumeric characters. |
| `description` | `descr` | `str` | — | — | Specifies the description of a policy component. |
| `encap` | `encap` | `str` | — | — | The in-band access encapsulation. |
