<!--
Generated by niwaki generate_docs — do not edit manually.
Re-generate: uv run python -m niwaki._codegen.generate_docs
-->

(vocab-tenant-vrf-pim6-static_rp-rp_entry)=

# `tenant.vrf.pim6.static_rp.rp_entry`

Static RP entry policy

| | |
| --- | --- |
| ACI class | `pimStaticRPEntryPol` |
| RN | `staticrpent-[{static_rp_address}]` |
| Cursor | `Pim6StaticRpRpEntryCursor` |
| Parent | {ref}`tenant.vrf.pim6.static_rp <vocab-tenant-vrf-pim6-static_rp>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.rp_entry(static_rp_address)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.

| parameter | wire | type | values | default | description |
| --- | --- | --- | --- | --- | --- |
| `static_rp_address` *(positional)* | `rpIp` | `str` | — | — | — |
| `description` | `descr` | `str` | — | — | Specifies the description of a policy component. |
| `name` | `name` | `str` | — | — | — |

## Children

| maker | creates | position |
| --- | --- | --- |
| `.group_range(**attrs)` | `pimRPGrpRangePol` | {ref}`tenant.vrf.pim6.static_rp.rp_entry.group_range <vocab-tenant-vrf-pim6-static_rp-rp_entry-group_range>` |

## APIC diagnostics

States the APIC can flag on this class — it accepts the configuration and reports the inconsistency afterwards.

**Config issues**

| code | meaning |
| --- | --- |
| `csw-invalid-pod-id` | Pod ID associated with this CSW Entry does not exist |
| `intervrf-route-map-empty` | No routemap associated with InterVRF policy / associated routemap is empty |
| `missing-rp-group-range` | missing RP Group range configuration. |
| `pim-not-supported-in-vpc-mode` | PIM is not supported in VPC mode |
| `pim-not-supported-on-ext-svi` | PIMv6 is not supported on external SVI interfaces |
| `pim-not-supported-on-floating-svi` | PIM is not supported on floating SVIs |
| `src-only-route-map-allowed` | Only source-based route-maps are allowed for BSR/Auto RP. |

**Faults**

| code | condition |
| --- | --- |
| `F2299` | `fltPimStaticRPEntryPolConfIssue` |
