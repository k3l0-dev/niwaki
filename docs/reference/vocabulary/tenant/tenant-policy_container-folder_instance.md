...
(vocab-tenant-policy_container-folder_instance)=

# `tenant.policy_container.folder_instance`

A folder instance configured by the administrator.

| | |
| --- | --- |
| ACI class | `vnsFolderInst` |
| RN | `FI_C-{ctrct_name_or_lbl}-G-{graph_name_or_lbl}-F-{node_name_or_lbl}-N-{name}` |
| Cursor | `PolicyContainerFolderInstanceCursor` |
| Parent | {ref}`tenant.policy_container <vocab-tenant-policy_container>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.folder_instance(ctrct_name_or_lbl, graph_name_or_lbl, node_name_or_lbl, name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.

| parameter | wire | type | values | default | description |
| --- | --- | --- | --- | --- | --- |
| `ctrct_name_or_lbl` *(positional)* | `ctrctNameOrLbl` | `str` | — | — | The folder instance contract name or label. |
| `graph_name_or_lbl` *(positional)* | `graphNameOrLbl` | `str` | — | — | The folder instance graph name or label. |
| `node_name_or_lbl` *(positional)* | `nodeNameOrLbl` | `str` | — | — | The folder instance node name or label. |
| `name` *(positional)* | `name` | `str` | — | — | The folder instance name. |
...
| `meta_folder_key` | `key` | `str` | — | — | The key uniquely identifying this configuration object. |
...
| --- | --- | --- |
| `.param_instance(name, **attrs)` | `vnsParamInst` | {ref}`tenant.policy_container.folder_instance.param_instance <vocab-tenant-policy_container-folder_instance-param_instance>` |
| `.relation(name, **attrs)` | `vnsCfgRelInst` | {ref}`tenant.policy_container.folder_instance.relation <vocab-tenant-policy_container-folder_instance-relation>` |
...
| code | condition |
| --- | --- |
| `F1377` | `fltVnsFolderInstValidationFailed` |
