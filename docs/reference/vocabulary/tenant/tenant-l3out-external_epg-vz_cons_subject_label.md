...
(vocab-tenant-l3out-external_epg-vz_cons_subject_label)=
...
| RN | `conssubjlbl-{name}` |
| Cursor | `VzConsSubjectLabelCursor` |
| Parent | {ref}`tenant.l3out.external_epg <vocab-tenant-l3out-external_epg>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.vz_cons_subject_label(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
