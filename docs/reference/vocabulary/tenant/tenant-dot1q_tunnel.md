...
| ACI class | `fvTnlEPg` |
| RN | `Tnlepg-{name}` |
| Cursor | `Dot1qTunnelCursor` |
| Parent | {ref}`tenant <vocab-tenant>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.dot1q_tunnel(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.

| parameter | wire | type | values | default | description |
| --- | --- | --- | --- | --- | --- |
| `name` *(positional)* | `name` | `str` | — | — | The name of the object. |
| `access_encapsulation` | `accEncap` | `str` | — | — | Access encap (vlan or vnid) |
