...
(vocab-tenant-chassis)=

# `tenant.chassis`

| | |
| --- | --- |
| ACI class | `vnsChassis` |
| RN | `chassis-{name}` |
| Cursor | `ChassisCursor` |
| Parent | {ref}`tenant <vocab-tenant>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.chassis(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
...
| --- | --- | --- |
| `.credentials(**attrs)` | `vnsCCred` | {ref}`tenant.chassis.credentials <vocab-tenant-chassis-credentials>` |
| `.management_interface(ip_address, **attrs)` | `vnsCMgmts` | {ref}`tenant.chassis.management_interface <vocab-tenant-chassis-management_interface>` |
