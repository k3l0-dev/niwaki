...
(vocab-tenant-logical_device-logical_interface)=

# `tenant.logical_device.logical_interface`
...
| ACI class | `vnsLIf` |
| RN | `lIf-{name}` |
| Cursor | `LogicalInterfaceCursor` |
| Parent | {ref}`tenant.logical_device <vocab-tenant-logical_device>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.logical_interface(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.

| parameter | wire | type | values | default | description |
| --- | --- | --- | --- | --- | --- |
| `name` *(positional)* | `name` | `str` | — | — | The name of the logical interface in the L4-L7 device cluster. |
| `encap` | `encap` | `str` | — | — | Static Encap |
| `enhanced_lag_policy_name` | `lagPolicyName` | `str` | — | — | LAG Policy Name |
