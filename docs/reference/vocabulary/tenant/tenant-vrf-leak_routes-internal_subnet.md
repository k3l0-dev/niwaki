...
(vocab-tenant-vrf-leak_routes-internal_subnet)=

# `tenant.vrf.leak_routes.internal_subnet`

| | |
| --- | --- |
| ACI class | `leakInternalSubnet` |
| RN | `leakintsubnet-[{subnet}]` |
| Cursor | `InternalSubnetCursor` |
| Parent | {ref}`tenant.vrf.leak_routes <vocab-tenant-vrf-leak_routes>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.internal_subnet(subnet)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
...
| --- | --- | --- |
| `.leak_to(leak_to_tenant_name, leak_to_vrf_name, **attrs)` | `leakTo` | {ref}`tenant.vrf.leak_routes.internal_subnet.leak_to <vocab-tenant-vrf-leak_routes-internal_subnet-leak_to>` |
