...
(vocab-tenant-access_function_profile-access_function_provider)=
...
| ACI class | `infraPEPg` |
| RN | `pepg-{name}` |
| Cursor | `AccessFunctionProviderCursor` |
| Parent | {ref}`tenant.access_function_profile <vocab-tenant-access_function_profile>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.access_function_provider(name)` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
