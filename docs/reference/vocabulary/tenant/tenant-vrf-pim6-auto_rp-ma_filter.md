...
(vocab-tenant-vrf-pim6-auto_rp-ma_filter)=

# `tenant.vrf.pim6.auto_rp.ma_filter`

Mapping agent (MA) filter policy.

| | |
| --- | --- |
| ACI class | `pimMAFilterPol` |
| RN | `mafilter` |
| Cursor | `Pim6AutoRpMaFilterCursor` |
| Parent | {ref}`tenant.vrf.pim6.auto_rp <vocab-tenant-vrf-pim6-auto_rp>` |
| Atomic | — |

## Attributes

Accepted as keyword arguments of `.ma_filter()` and of `.set(**attrs)` on this cursor.  Enum parameters also accept the plain string.
