...
class qosLlfcIfPol(ManagedObject):
    """ACI Managed Object: ``qosLlfcIfPol`` — Interface Link Level Flow Control Policy.

    RN format: ``llfc-{name}``
    """

    _aci_class: ClassVar[str] = "qosLlfcIfPol"
    _rn_format: ClassVar[str] = "llfc-{name}"
    ...
    link_level_flow_control_receive_mode: L1LinkLevelFlowCtrl = Field(
        default=L1LinkLevelFlowCtrl.OFF, alias="llfcRcvAdminSt"
    )
    link_level_flow_control_send_mode: L1LinkLevelFlowCtrl = Field(
        default=L1LinkLevelFlowCtrl.OFF, alias="llfcSendAdminSt"
    )
