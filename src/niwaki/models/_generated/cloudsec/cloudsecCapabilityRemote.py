# ruff: noqa
# Generated by niwaki codegen — do not edit manually.
from __future__ import annotations

from typing import ClassVar, Annotated
from pydantic import Field

from niwaki.models._generated.enums.CloudsecCapBm import CloudsecCapBm
from niwaki.models._generated.enums.CloudsecCloudsecStatus import CloudsecCloudsecStatus
from niwaki.models._generated.enums.CloudsecVersion import CloudsecVersion

from niwaki.models.base import ManagedObject


class cloudsecCapabilityRemote(ManagedObject):
    """ACI Managed Object: ``cloudsecCapabilityRemote`` — Cloudsec Local Site Capability exchanged with MSC.

    RN format: ``cloudseccapremote``
    """

    _aci_class: ClassVar[str] = "cloudsecCapabilityRemote"
    _rn_format: ClassVar[str] = "cloudseccapremote"
    _naming_props: ClassVar[list[str]] = []
    _contains: ClassVar[frozenset[str]] = frozenset(
        {
            "aaaRbacAnnotation",
            "tagAnnotation",
            "tagTag",
        }
    )

    # ── Semantic metadata (from APIC schema) ───────────────────────────────────
    _mo_category: ClassVar[str] = "Regular"
    _write_access: ClassVar[frozenset[str]] = frozenset(["admin", "tenant-ext-connectivity"])
    _is_observable: ClassVar[bool] = False
    _is_faultable: ClassVar[bool] = False
    _is_health_scorable: ClassVar[bool] = False
    _has_stats: ClassVar[bool] = False

    # ── Configurable ───────────────────────────────────────────────────────────
    annotation: Annotated[str, Field(max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    cap_bm: CloudsecCapBm = Field(default=CloudsecCapBm.NONE, alias="capBm")
    cloudsec_status: CloudsecCloudsecStatus = Field(
        default=CloudsecCloudsecStatus.DISABLED, alias="cloudsecStatus"
    )
    description: Annotated[
        str,
        Field(max_length=128, pattern="^[a-zA-Z0-9\\\\!#$%()*,-./:;@ _{|}~?&+]+$", alias="descr"),
    ] = ""
    name: Annotated[str, Field(max_length=64, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    display_name: Annotated[
        str, Field(max_length=63, pattern="^[a-zA-Z0-9_.-]+$", alias="nameAlias")
    ] = ""
    owner_key: Annotated[
        str,
        Field(
            max_length=128, pattern="^[a-zA-Z0-9\\\\!#$%()*,-./:;@ _{|}~?&+]+$", alias="ownerKey"
        ),
    ] = ""
    owner_tag: Annotated[
        str,
        Field(max_length=64, pattern="^[a-zA-Z0-9\\\\!#$%()*,-./:;@ _{|}~?&+]+$", alias="ownerTag"),
    ] = ""
    userdom: Annotated[str, Field(max_length=1024, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    version: CloudsecVersion = CloudsecVersion.V1
