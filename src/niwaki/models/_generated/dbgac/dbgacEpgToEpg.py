...
class dbgacEpgToEpg(ManagedObject):
    """ACI Managed Object: ``dbgacEpgToEpg`` — Application Component to Aplication Component Atomic Counter Policy.
    ...
    RN format: ``epgToEpg-{name}``
    """

    _aci_class: ClassVar[str] = "dbgacEpgToEpg"
    _rn_format: ClassVar[str] = "epgToEpg-{name}"
    ...
    name: Annotated[
        ...
        Field(
            ...
            max_length=64,
            pattern="^[a-zA-Z0-9_.:-]+$",
            description="The name of the endpoint group-to-endpoint group policy definition. This name can be between 1 and 64 alphanumeric characters. Note that you cannot change this name after the object has been saved.",
        ),
    ]

    # ── Create-only (ignored by APIC on modification) ─────────────────────────
    ip_conn_type: Annotated[Flags[AcIpType], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({AcIpType.IPV4}),
        validation_alias="ipConnType",
        serialization_alias="ipConnType",
    )
