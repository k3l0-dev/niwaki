# ruff: noqa
# Generated by niwaki codegen — do not edit manually.
from __future__ import annotations

from typing import ClassVar, Annotated
from pydantic import Field

from niwaki.models.base import ManagedObject


class faultThrValueSint32(ManagedObject):
    """ACI Managed Object: ``faultThrValueSint32``.

    The threshold value Service Interface Notification Statistics (SINT) 32.

    RN format: ``thrValSint32-{property}``
    """

    _aci_class: ClassVar[str] = "faultThrValueSint32"
    _rn_format: ClassVar[str] = "thrValSint32-{property}"
    _naming_props: ClassVar[list[str]] = ["property"]
    _contains: ClassVar[frozenset[str]] = frozenset(
        {
            "aaaRbacAnnotation",
            "tagAnnotation",
            "tagTag",
        }
    )

    # ── Semantic metadata (from APIC schema) ───────────────────────────────────
    _mo_category: ClassVar[str] = "Regular"
    _write_access: ClassVar[frozenset[str]] = frozenset([])
    _is_creatable: ClassVar[bool] = True
    _is_observable: ClassVar[bool] = False
    _is_faultable: ClassVar[bool] = False
    _is_health_scorable: ClassVar[bool] = False
    _has_stats: ClassVar[bool] = False

    # ── Naming (required) ──────────────────────────────────────────────────────
    property: Annotated[str, Field(validation_alias="propId", serialization_alias="propId")]

    # ── Configurable ───────────────────────────────────────────────────────────
    annotation: Annotated[
        str,
        Field(
            max_length=128,
            pattern="^[a-zA-Z0-9_.:-]+$",
            description="User annotation. Suggested format orchestrator:value",
        ),
    ] = ""
    threshold_value_from_policy: Annotated[
        int,
        Field(
            validation_alias="thrValue",
            serialization_alias="thrValue",
            description="Threshold value from policy.",
        ),
    ] = 0
    userdom: Annotated[str, Field(max_length=1024, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    value: Annotated[int, Field(description="Value of the property.")] = 0
