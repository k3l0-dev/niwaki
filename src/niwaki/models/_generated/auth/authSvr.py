# ruff: noqa
# Generated by niwaki codegen — do not edit manually.
from __future__ import annotations

from typing import ClassVar, Annotated
from pydantic import Field

from niwaki.models._generated.enums.AuthMode import AuthMode
from niwaki.models._generated.enums.AuthScope import AuthScope
from niwaki.models._generated.enums.AuthTrigSt import AuthTrigSt
from niwaki.models._generated.enums.VmmStatsMode import VmmStatsMode

from niwaki.models.base import ManagedObject


class authSvr(ManagedObject):
    """ACI Managed Object: ``authSvr`` — Auth Server.

    RN format: ``authsvr-{name}``
    """

    _aci_class: ClassVar[str] = "authSvr"
    _rn_format: ClassVar[str] = "authsvr-{name}"
    _naming_props: ClassVar[list[str]] = ["name"]
    _contains: ClassVar[frozenset[str]] = frozenset(
        {
            "aaaRbacAnnotation",
            "authRsSvrToMgmtEPg",
            "authRsSvrToMonPol",
            "authUsrAccP",
            "faultInst",
            "tagAnnotation",
            "tagExtMngdInst",
            "tagInst",
            "tagTag",
        }
    )

    # ── Semantic metadata (from APIC schema) ───────────────────────────────────
    _mo_category: ClassVar[str] = "Regular"
    _write_access: ClassVar[frozenset[str]] = frozenset(["admin"])
    _is_observable: ClassVar[bool] = True
    _is_faultable: ClassVar[bool] = True
    _is_health_scorable: ClassVar[bool] = True
    _has_stats: ClassVar[bool] = False

    # ── Naming (required) ──────────────────────────────────────────────────────
    name: Annotated[str, Field(min_length=1, max_length=64, pattern="^[a-zA-Z0-9_.:-]+$")]

    # ── Create-only (ignored by APIC on modification) ─────────────────────────
    hostname_or_ip_address: str = Field(default="", alias="hostOrIp")

    # ── Configurable ───────────────────────────────────────────────────────────
    annotation: Annotated[str, Field(max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    event_sync_interval_seconds: str = Field(default="", alias="evtSyncIntvl")
    triggered_inventory_sync_status: AuthTrigSt = Field(
        default=AuthTrigSt.UNTRIGGERED, alias="inventoryTrigSt"
    )
    mode: AuthMode = AuthMode.AD
    display_name: Annotated[
        str, Field(max_length=63, pattern="^[a-zA-Z0-9_.-]+$", alias="nameAlias")
    ] = ""
    port: str = ""
    datacenter: Annotated[str, Field(max_length=512, alias="rootContName")] = ""
    scope: AuthScope = AuthScope.ISE
    sequence_number: Annotated[int, Field(alias="seqNum")] = 0
    stats_option: VmmStatsMode = Field(default=VmmStatsMode.DISABLED, alias="statsMode")
    userdom: Annotated[str, Field(max_length=1024, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
