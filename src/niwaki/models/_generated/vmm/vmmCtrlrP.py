# ruff: noqa
# Generated by niwaki codegen — do not edit manually.
from __future__ import annotations

from typing import ClassVar, Annotated
from pydantic import BeforeValidator, Field

from niwaki.models._wire import Flags, parse_flags
from niwaki.models._generated.enums.CompmsftConfigIssues import CompmsftConfigIssues
from niwaki.models._generated.enums.VmmAosVersion import VmmAosVersion
from niwaki.models._generated.enums.VmmDvsVersion import VmmDvsVersion
from niwaki.models._generated.enums.VmmMode import VmmMode
from niwaki.models._generated.enums.VmmN1KVStatsMode import VmmN1KVStatsMode
from niwaki.models._generated.enums.VmmScope import VmmScope
from niwaki.models._generated.enums.VmmStatsMode import VmmStatsMode
from niwaki.models._generated.enums.VmmTrigSt import VmmTrigSt
from niwaki.models._generated.enums.VmmVxlanDeplPref import VmmVxlanDeplPref

from niwaki.models.base import ManagedObject


class vmmCtrlrP(ManagedObject):
    """ACI Managed Object: ``vmmCtrlrP`` — VMM Controller.

    The VMM controller profile specifies how to connect to a single VM management controller that is part of a policy enforcement domain. For example, the VMM controller profile could be a policy to connect a VMware vCenter that is part of a VMM domain.

    RN format: ``ctrlr-{name}``

    The APIC can flag these accepted-but-inconsistent states on this class
    (read-only ``configIssues``):

    - ``aaacert-invalid`` — SCVMM comp inventory is missing due to invalid Certificate on SCVMM server or aaaVMMCertificateRule is missing on APIC controller.
    - ``domain-not-deployable`` — Parent Domain is not deployable due to configuration issues.
    - ``duplicate-mac-in-inventory`` — SCVMM Agent found duplicate mac address in SCVMM comp Inventory.
    - ``duplicate-rootContName`` — Controller is not deployable. the same rootContName controller also exist in another vmm Domain.
    - ``enfpref-not-set`` — Switching Preference not set yet.
    - ``invalid-CtrlrAssoc`` — Controller is not associated to a valid VM Controller. Please provide the association.
    - ``invalid-cluster-ctrlr`` — There are config issues on cluster ctrlr.
    - ``invalid-mcastAddr`` — No valid Multicast Address Block specified for the Controller.
    - ``invalid-object-in-inventory`` — SCVMM Agent has ignored duplicate object(s) in SCVMM comp Inventory .
    - ``invalid-policy`` — MAC-PINNING POLICY basing on physical-NIC load is not supported for this mode.
    - ``invalid-rootContName`` — SCVMM controller does not have cloudName that matches with rootContName. Controller is not deployable.
    - ``invalid-statsP`` — Controller is not associated to a valid Statistics Collection Policy
    - ``invalid-usraccp`` — Controller is not associated to a valid Access Profile
    - ``invalid-vxlanNs`` — No valid VXLAN Pool specified for the Controller.
    - ``inventory-failed`` — SCVMM Agent failed to post comp inventory to APIC Controller.
    - ``missing-cluster-ctrlr`` — Nutanix domain should have a valid cluster controller association.
    - ``missing-hostGroup-in-cloud`` — No valid hostGroup exist under the cloud resource in SCVMM Controller. Controller is not deployable
    - ``missing-infra-vlan`` — Missing infra VLAN for the controller.
    - ``missing-ns-ctrlr-assoc`` — vCenter should be associated to a vShield or the parent domain should be associated with a VLAN pool.
    - ``missing-rootContName`` — SCVMM controller has missing cloudName that matches with rootContName but Its SCVMM HostGroup is still exists along with APIC VMMdomain Logical switch.
    - ``mode-not-set`` — Mode not set yet.
    - ``zero-mac-in-inventory`` — SCVMM Agent found zero mac address in SCVMM comp Inventory.
    """

    _aci_class: ClassVar[str] = "vmmCtrlrP"
    _rn_format: ClassVar[str] = "ctrlr-{name}"
    _naming_props: ClassVar[list[str]] = ["name"]
    _config_issues: ClassVar[dict[str, str]] = {
        "aaacert-invalid": "SCVMM comp inventory is missing due to invalid Certificate on SCVMM server or aaaVMMCertificateRule is missing on APIC controller.",
        "domain-not-deployable": "Parent Domain is not deployable due to configuration issues.",
        "duplicate-mac-in-inventory": "SCVMM Agent found duplicate mac address in SCVMM comp Inventory.",
        "duplicate-rootContName": "Controller is not deployable. the same rootContName controller also exist in another vmm Domain.",
        "enfpref-not-set": "Switching Preference not set yet.",
        "invalid-cluster-ctrlr": "There are config issues on cluster ctrlr.",
        "invalid-CtrlrAssoc": "Controller is not associated to a valid VM Controller. Please provide the association.",
        "invalid-mcastAddr": "No valid Multicast Address Block specified for the Controller.",
        "invalid-object-in-inventory": "SCVMM Agent has ignored duplicate object(s) in SCVMM comp Inventory .",
        "invalid-policy": "MAC-PINNING POLICY basing on physical-NIC load is not supported for this mode.",
        "invalid-rootContName": "SCVMM controller does not have cloudName that matches with rootContName. Controller is not deployable.",
        "invalid-statsP": "Controller is not associated to a valid Statistics Collection Policy",
        "invalid-usraccp": "Controller is not associated to a valid Access Profile",
        "invalid-vxlanNs": "No valid VXLAN Pool specified for the Controller.",
        "inventory-failed": "SCVMM Agent failed to post comp inventory to APIC Controller.",
        "missing-cluster-ctrlr": "Nutanix domain should have a valid cluster controller association.",
        "missing-hostGroup-in-cloud": "No valid hostGroup exist under the cloud resource in SCVMM Controller. Controller is not deployable",
        "missing-infra-vlan": "Missing infra VLAN for the controller.",
        "missing-ns-ctrlr-assoc": "vCenter should be associated to a vShield or the parent domain should be associated with a VLAN pool.",
        "missing-rootContName": "SCVMM controller has missing cloudName that matches with rootContName but Its SCVMM HostGroup is still exists along with APIC VMMdomain Logical switch.",
        "mode-not-set": "Mode not set yet.",
        "not-applicable": "",
        "zero-mac-in-inventory": "SCVMM Agent found zero mac address in SCVMM comp Inventory.",
    }
    _fault_codes: ClassVar[dict[str, str]] = {
        "F0564": "fltVmmCtrlrPConfigFailed",
        "F1669": "fltVmmCtrlrPMsftConfigFailed",
    }
    _contains: ClassVar[frozenset[str]] = frozenset(
        {
            "aaaRbacAnnotation",
            "faultInst",
            "tagAnnotation",
            "tagExtMngdInst",
            "tagInst",
            "tagTag",
            "vmmAgtStatus",
            "vmmClusterCtrlrP",
            "vmmEpValidatorPol",
            "vmmHvAvailPol",
            "vmmPlInf",
            "vmmRsAcc",
            "vmmRsCtrlrPMonPol",
            "vmmRsMcastAddrNs",
            "vmmRsMgmtEPg",
            "vmmRsToExtDevMgr",
            "vmmRsVmmCtrlrP",
            "vmmRsVxlanNs",
        }
    )

    # ── Semantic metadata (from APIC schema) ───────────────────────────────────
    _mo_category: ClassVar[str] = "Regular"
    _write_access: ClassVar[frozenset[str]] = frozenset(["admin", "vmm-policy"])
    _is_creatable: ClassVar[bool] = True
    _is_observable: ClassVar[bool] = True
    _is_faultable: ClassVar[bool] = True
    _is_health_scorable: ClassVar[bool] = True
    _has_stats: ClassVar[bool] = False

    # ── Naming (required) ──────────────────────────────────────────────────────
    name: Annotated[
        str,
        Field(
            min_length=1,
            max_length=64,
            pattern="^[a-zA-Z0-9_.:-]+$",
            description="The name of the controller profile.",
        ),
    ]

    # ── Create-only (ignored by APIC on modification) ─────────────────────────
    hostname_or_ip_address: str = Field(
        default="",
        validation_alias="hostOrIp",
        serialization_alias="hostOrIp",
        description="Host or IP",
    )

    # ── Configurable ───────────────────────────────────────────────────────────
    annotation: Annotated[
        str,
        Field(
            max_length=128,
            pattern="^[a-zA-Z0-9_.:-]+$",
            description="User annotation. Suggested format orchestrator:value",
        ),
    ] = ""
    aos_version: VmmAosVersion = Field(
        default=VmmAosVersion.UNKNOWN,
        validation_alias="aosVersion",
        serialization_alias="aosVersion",
        description="version of aos",
    )
    dvs_version: VmmDvsVersion = Field(
        default=VmmDvsVersion.UNMANAGED,
        validation_alias="dvsVersion",
        serialization_alias="dvsVersion",
    )
    triggered_inventory_sync_status: VmmTrigSt = Field(
        default=VmmTrigSt.UNTRIGGERED,
        validation_alias="inventoryTrigSt",
        serialization_alias="inventoryTrigSt",
    )
    mode: VmmMode = Field(default=VmmMode.DEFAULT, description="The mode of operation.")
    msft_config_err_msg: Annotated[
        str,
        Field(
            max_length=256,
            validation_alias="msftConfigErrMsg",
            serialization_alias="msftConfigErrMsg",
            description="Deployment Error Message of Mirosoft Plugin SCVM Controller. It captures error message encountered in SCVMM Controller plugin.This error message represents specific details for bitmask based msftConfigIssues fault.",
        ),
    ] = ""
    configissues: Annotated[Flags[CompmsftConfigIssues], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({CompmsftConfigIssues.NOT_APPLICABLE}),
        validation_alias="msftConfigIssues",
        serialization_alias="msftConfigIssues",
    )
    n1kv_stats_mode: VmmN1KVStatsMode = Field(
        default=VmmN1KVStatsMode.ENABLED,
        validation_alias="n1kvStatsMode",
        serialization_alias="n1kvStatsMode",
        description="n1kv statistics enable",
    )
    display_name: Annotated[
        str,
        Field(
            max_length=63,
            pattern="^[a-zA-Z0-9_.-]+$",
            validation_alias="nameAlias",
            serialization_alias="nameAlias",
        ),
    ] = ""
    port: Annotated[int, Field(ge=0, le=65535, description="Port")] = 0
    datacenter: Annotated[
        str,
        Field(
            max_length=512,
            validation_alias="rootContName",
            serialization_alias="rootContName",
            description="Top level container name.",
        ),
    ] = ""
    type: VmmScope = Field(
        default=VmmScope.VM,
        validation_alias="scope",
        serialization_alias="scope",
        description="The VMM control policy scope.",
    )
    seq_num: Annotated[
        int,
        Field(
            validation_alias="seqNum",
            serialization_alias="seqNum",
            description="An ISIS link-state packet sequence number.",
        ),
    ] = 0
    stats_collection: VmmStatsMode = Field(
        default=VmmStatsMode.DISABLED,
        validation_alias="statsMode",
        serialization_alias="statsMode",
        description="The statistics mode.",
    )
    userdom: Annotated[str, Field(max_length=1024, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    vxlan_deployment_preference: VmmVxlanDeplPref = Field(
        default=VmmVxlanDeplPref.VXLAN,
        validation_alias="vxlanDeplPref",
        serialization_alias="vxlanDeplPref",
    )
