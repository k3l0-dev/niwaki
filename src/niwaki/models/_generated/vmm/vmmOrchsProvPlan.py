# ruff: noqa
# Generated by niwaki codegen — do not edit manually.
from __future__ import annotations

from typing import ClassVar, Annotated
from pydantic import Field

from niwaki.models.base import ManagedObject


class vmmOrchsProvPlan(ManagedObject):
    """ACI Managed Object: ``vmmOrchsProvPlan`` — Orchestrator Plans for this VMMDom.

    RN format: ``plan-{name}``
    """

    _aci_class: ClassVar[str] = "vmmOrchsProvPlan"
    _rn_format: ClassVar[str] = "plan-{name}"
    _naming_props: ClassVar[list[str]] = ["name"]
    _contains: ClassVar[frozenset[str]] = frozenset(
        {
            "aaaRbacAnnotation",
            "tagAnnotation",
            "tagTag",
            "vmmOrchsProvPlanFW",
            "vmmOrchsProvPlanLB",
            "vmmOrchsProvPlanSrvc",
        }
    )

    # ── Semantic metadata (from APIC schema) ───────────────────────────────────
    _mo_category: ClassVar[str] = "Regular"
    _write_access: ClassVar[frozenset[str]] = frozenset(["admin", "vmm-policy"])
    _is_observable: ClassVar[bool] = False
    _is_faultable: ClassVar[bool] = False
    _is_health_scorable: ClassVar[bool] = False
    _has_stats: ClassVar[bool] = False

    # ── Naming (required) ──────────────────────────────────────────────────────
    name: Annotated[str, Field(min_length=1, max_length=16, pattern="^[a-zA-Z0-9_.:-]+$")]

    # ── Configurable ───────────────────────────────────────────────────────────
    annotation: Annotated[str, Field(max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    cre_shrd_svc: bool = Field(default=False, alias="creShrdSvc")
    map_fv_tenant: bool = Field(default=False, alias="mapFvTenant")
    maximum_number_of_bd_allowed_per_tenant: Annotated[
        int, Field(ge=1, le=4000, alias="maxBDAllowed")
    ] = 4000
    maximum_number_of_vrf_allowed_per_tenant: Annotated[
        int, Field(ge=1, le=4000, alias="maxCtxAllowed")
    ] = 4000
    maximum_number_of_epg_allowed_per_tenant: Annotated[
        int, Field(ge=1, le=4000, alias="maxEPGAllowed")
    ] = 4000
    max_public_ip_allowed: Annotated[int, Field(ge=1, le=4000, alias="maxPublicIPAllowed")] = 4000
    display_name: Annotated[
        str, Field(max_length=63, pattern="^[a-zA-Z0-9_.-]+$", alias="nameAlias")
    ] = ""
    shared_bd: str = Field(default="", alias="sharedBD")
    shared_dhcp_epg: str = Field(default="", alias="sharedDhcpEpg")
    shared_dns_epg: str = Field(default="", alias="sharedDnsEpg")
    svc_rsrc_pool_name: Annotated[str, Field(max_length=512, alias="svcRsrcPoolName")] = ""
    userdom: Annotated[str, Field(max_length=1024, pattern="^[a-zA-Z0-9_.:-]+$")] = ""
    vpc_enabled: bool = Field(default=False, alias="vpcEnabled")
