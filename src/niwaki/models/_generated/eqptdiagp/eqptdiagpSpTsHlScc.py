...
class eqptdiagpSpTsHlScc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsHlScc`` — Spine Sys-controller-card Health Diags.

    The ongoing diagnostic health test set to run at bootup on spine system controller cards.

    RN format: ``spHlSccDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsHlScc"
    _rn_format: ClassVar[str] = "spHlSccDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagSpHlSccTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpHlSccTestBmT.NONE}),
    ...
    full_bm: Annotated[Flags[EqptdiagSpHlSccTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpHlSccTestBmT.NONE}),
