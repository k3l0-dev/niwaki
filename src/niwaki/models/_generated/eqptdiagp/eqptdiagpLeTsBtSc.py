...
class eqptdiagpLeTsBtSc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpLeTsBtSc`` — Bootup Diag Policy for the Leaf Node Supervisor Line Cards.

    The diagnostic test set for leaf fabric nodes to run at bootup on supervisor cards.

    RN format: ``leBtSupDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpLeTsBtSc"
    _rn_format: ClassVar[str] = "leBtSupDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagLeBtSupTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeBtSupTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_bm: Annotated[Flags[EqptdiagLeBtSupTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeBtSupTestBmT.NONE}),
