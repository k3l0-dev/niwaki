...
class eqptdiagpLeTsBtLc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpLeTsBtLc`` — Leaf Linecard Bootup Diags.

    The diagnostic test set for leaf fabric nodes to run at bootup on line cards (I/O cards).

    RN format: ``leBtLcDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpLeTsBtLc"
    _rn_format: ClassVar[str] = "leBtLcDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagLeBtLcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeBtLcTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_bm: Annotated[Flags[EqptdiagLeBtLcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeBtLcTestBmT.NONE}),
