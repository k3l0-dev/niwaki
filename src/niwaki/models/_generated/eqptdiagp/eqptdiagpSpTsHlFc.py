...
class eqptdiagpSpTsHlFc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsHlFc`` — Spine Fabric-card Health Diags.

    The ongoing diagnostic health test set to run at bootup on spine fabric cards.

    RN format: ``spHlFcDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsHlFc"
    _rn_format: ClassVar[str] = "spHlFcDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagSpHlFcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpHlFcTestBmT.NONE}),
    ...
    full_bm: Annotated[Flags[EqptdiagSpHlFcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpHlFcTestBmT.NONE}),
