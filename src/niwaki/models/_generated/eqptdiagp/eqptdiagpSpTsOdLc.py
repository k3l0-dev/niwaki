...
class eqptdiagpSpTsOdLc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsOdLc`` — Spine Linecard On-demand Diags.

    The diagnostic on-demand test set to run on spine line cards (I/O cards).

    RN format: ``spOdLcDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsOdLc"
    _rn_format: ClassVar[str] = "spOdLcDiags-{name}"
    ...
    bitmask_of_diagnostics_tests_to_be_run: Annotated[
        Flags[EqptdiagSpOdLcTestBmT], BeforeValidator(parse_flags)
    ] = Field(
        default_factory=lambda: frozenset({EqptdiagSpOdLcTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_diagnostics: Annotated[Flags[EqptdiagSpOdLcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpOdLcTestBmT.NONE}),
