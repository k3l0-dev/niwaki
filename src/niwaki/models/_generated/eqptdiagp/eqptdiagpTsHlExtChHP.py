...
class eqptdiagpTsHlExtChHP(ManagedObject):
    """ACI Managed Object: ``eqptdiagpTsHlExtChHP`` — Health Diag Policy Set for ExtCh (fex) Host Port Types.

    RN format: ``hlEHpDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpTsHlExtChHP"
    _rn_format: ClassVar[str] = "hlEHpDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagHlExtChHPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagHlExtChHPTestBmT.NONE}),
    ...
    full_bm: Annotated[Flags[EqptdiagHlExtChHPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagHlExtChHPTestBmT.NONE}),
