...
class eqptdiagpSpTsOdSc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsOdSc`` — Spine Sup-card On-demand Diags.

    The diagnostic on-demand test set for leaf fabric nodes to run on spine supervisor cards.

    RN format: ``spOdScDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsOdSc"
    _rn_format: ClassVar[str] = "spOdScDiags-{name}"
    ...
    bitmask_of_diagnostics_tests_to_be_run: Annotated[
        Flags[EqptdiagSpOdScTestBmT], BeforeValidator(parse_flags)
    ] = Field(
        default_factory=lambda: frozenset({EqptdiagSpOdScTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_diagnostics: Annotated[Flags[EqptdiagSpOdScTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpOdScTestBmT.NONE}),
