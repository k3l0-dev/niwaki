...
class eqptdiagpTsBtFabP(ManagedObject):
    """ACI Managed Object: ``eqptdiagpTsBtFabP`` — Bootup Diag Policy Set for Fabric Port Types.
    ...
    _aci_class: ClassVar[str] = "eqptdiagpTsBtFabP"
    _rn_format: ClassVar[str] = "btFpDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagBtFabPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagBtFabPTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_bm: Annotated[Flags[EqptdiagBtFabPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagBtFabPTestBmT.NONE}),
