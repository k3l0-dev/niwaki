...
class eqptdiagpTsHlFabP(ManagedObject):
    """ACI Managed Object: ``eqptdiagpTsHlFabP`` — Health Diag Policy Set for Fab Port Types.

    The ongoing diagnostic health test set for fabric ports.

    RN format: ``hlFpDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpTsHlFabP"
    _rn_format: ClassVar[str] = "hlFpDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagHlFabPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagHlFabPTestBmT.NONE}),
    ...
    full_bm: Annotated[Flags[EqptdiagHlFabPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagHlFabPTestBmT.NONE}),
