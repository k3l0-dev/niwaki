...
class eqptdiagpSpTsHlSc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsHlSc`` — Spine Sup-card Health Diags.

    The ongoing diagnostic health test set to run at bootup on spine supervisor cards.

    RN format: ``spHlScDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsHlSc"
    _rn_format: ClassVar[str] = "spHlScDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagSpHlScTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpHlScTestBmT.NONE}),
    ...
    full_bm: Annotated[Flags[EqptdiagSpHlScTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpHlScTestBmT.NONE}),
