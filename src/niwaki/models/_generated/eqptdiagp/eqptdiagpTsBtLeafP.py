...
class eqptdiagpTsBtLeafP(ManagedObject):
    """ACI Managed Object: ``eqptdiagpTsBtLeafP`` — Bootup Diag Policy Set for Leaf Port Types.

    The diagnostic test set for leaf fabric ports to run at bootup on line cards (I/O cards).

    RN format: ``btLpDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpTsBtLeafP"
    _rn_format: ClassVar[str] = "btLpDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagBtLeafPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagBtLeafPTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_bm: Annotated[Flags[EqptdiagBtLeafPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagBtLeafPTestBmT.NONE}),
