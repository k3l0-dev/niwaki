...
class eqptdiagpSpTsOdFc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsOdFc`` — Spine Fabric-card On-demand Diags.

    The diagnostic on-demand test set to run on spine fabric cards.

    RN format: ``spOdFcDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsOdFc"
    _rn_format: ClassVar[str] = "spOdFcDiags-{name}"
    ...
    bitmask_of_diagnostics_tests_to_be_run: Annotated[
        Flags[EqptdiagSpOdFcTestBmT], BeforeValidator(parse_flags)
    ] = Field(
        default_factory=lambda: frozenset({EqptdiagSpOdFcTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_diagnostics: Annotated[Flags[EqptdiagSpOdFcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpOdFcTestBmT.NONE}),
