...
class eqptdiagpSpTsBtLc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsBtLc`` — Spine Linecard Bootup Diags.

    The diagnostic test set to run at bootup on spine line cards (I/O cards).

    RN format: ``spBtLcDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsBtLc"
    _rn_format: ClassVar[str] = "spBtLcDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagSpBtLcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpBtLcTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_bm: Annotated[Flags[EqptdiagSpBtLcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpBtLcTestBmT.NONE}),
