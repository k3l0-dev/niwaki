...
class eqptdiagpSpTsBtSc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsBtSc`` — Spine Sup-card Bootup Diags.

    The diagnostic test set to run at bootup on spine supervisor cards.

    RN format: ``spBtScDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsBtSc"
    _rn_format: ClassVar[str] = "spBtScDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagSpBtScTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpBtScTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_bm: Annotated[Flags[EqptdiagSpBtScTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpBtScTestBmT.NONE}),
