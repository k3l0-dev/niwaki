...
class eqptdiagpLeTsHlLc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpLeTsHlLc`` — Leaf Linecard Health Diags.

    The diagnostic ongoing health test set for leaf fabric nodes to run on line cards (I/O cards).

    RN format: ``leHlLcDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpLeTsHlLc"
    _rn_format: ClassVar[str] = "leHlLcDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagLeHlLcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeHlLcTestBmT.NONE}),
    ...
    full_bm: Annotated[Flags[EqptdiagLeHlLcTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeHlLcTestBmT.NONE}),
