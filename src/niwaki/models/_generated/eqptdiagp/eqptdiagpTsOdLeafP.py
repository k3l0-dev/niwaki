...
class eqptdiagpTsOdLeafP(ManagedObject):
    """ACI Managed Object: ``eqptdiagpTsOdLeafP`` — On-Demand Diag Policy Set for Leaf Port Types.

    The diagnostic on-demand test set to run on leaf fabric ports.

    RN format: ``odLpDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpTsOdLeafP"
    _rn_format: ClassVar[str] = "odLpDiags-{name}"
    ...
    _write_access: ClassVar[frozenset[str]] = frozenset(
        ["access-connectivity", "access-equipment", "admin", "custom-port-privilege", "ops"]
    )
    ...
    bitmask_of_diagnostics_tests_to_be_run: Annotated[
        Flags[EqptdiagOdLeafPTestBmT], BeforeValidator(parse_flags)
    ] = Field(
        default_factory=lambda: frozenset({EqptdiagOdLeafPTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_diagnostics: Annotated[Flags[EqptdiagOdLeafPTestBmT], BeforeValidator(parse_flags)] = (
        Field(
            default_factory=lambda: frozenset({EqptdiagOdLeafPTestBmT.NONE}),
