...
class eqptdiagpLeTsOdEcc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpLeTsOdEcc`` — FEX On-demand Diags.

    The diagnostic test set for leaf fabric nodes to run on extended chassis cards (FEXes).

    RN format: ``leOdEccDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpLeTsOdEcc"
    _rn_format: ClassVar[str] = "leOdEccDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagLeOdEccTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeOdEccTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask diagnostics test.",
    )
    full_bm: Annotated[Flags[EqptdiagLeOdEccTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagLeOdEccTestBmT.NONE}),
