...
class eqptdiagpTsHlLeafP(ManagedObject):
    """ACI Managed Object: ``eqptdiagpTsHlLeafP`` — Health Diag Policy Set for Leaf Port Types.

    The ongoing diagnostic tests for leaf fabric ports.

    RN format: ``hlLpDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpTsHlLeafP"
    _rn_format: ClassVar[str] = "hlLpDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagHlLeafPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagHlLeafPTestBmT.NONE}),
    ...
    full_bm: Annotated[Flags[EqptdiagHlLeafPTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagHlLeafPTestBmT.NONE}),
