...
class eqptdiagpSpTsBtScc(ManagedObject):
    """ACI Managed Object: ``eqptdiagpSpTsBtScc`` — Spine Sys-controller-card Bootup Diags.

    The diagnostic test set to run at bootup on spine system controller cards.

    RN format: ``spBtSccDiags-{name}``
    """

    _aci_class: ClassVar[str] = "eqptdiagpSpTsBtScc"
    _rn_format: ClassVar[str] = "spBtSccDiags-{name}"
    ...
    diags_bm: Annotated[Flags[EqptdiagSpBtSccTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpBtSccTestBmT.NONE}),
        validation_alias="diagsBm",
        serialization_alias="diagsBm",
        description="The bitmask of diagnostic test groups that are enabled.",
    )
    full_bm: Annotated[Flags[EqptdiagSpBtSccTestBmT], BeforeValidator(parse_flags)] = Field(
        default_factory=lambda: frozenset({EqptdiagSpBtSccTestBmT.NONE}),
